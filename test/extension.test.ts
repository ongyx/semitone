...
describe("extension", () => {
	describe("workspace.onDidChangeActiveTextEditor", () => {
		it("prompts to add a file to the matching project when opened", async () => {
			...
			assert.ok(folder)
			const td = await workspace.openTextDocument(
				Uri.joinPath(folder.uri, "src/HelloWorld.cs"),
			)
